...
5. Allow clients to join by calling start_to_listen().
g_NetCom.start_to_listen();
...
The Servers IP and port will be stored in the clientCom object and can is obtained by calling get_server_IP_and_port(string IP_and_port_container).
...
enum net_statuses{net_server,net_client};
...
class networkCom
{
    public:
        ...
        bool send_data(float* data_array,SOCKET SocReceiver);                       //only for SERVER
        bool recv_data(string& data_string);                                        //only for CLIENT
        bool recv_data(float* data_array);                                          //only for CLIENT
        bool recv_data(string& data_string,SOCKET SocSender);                       //only for SERVER
        bool recv_data(float* data_array,SOCKET SocSender);                         //only for SERVER
