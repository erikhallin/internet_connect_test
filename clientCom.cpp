...
bool clientCom::send_data(float* data_array)
{
    int val=send(m_SocServer,(char*)data_array,sizeof(float)*data_array[0],0);
...
{
    char buffer[256];
    int val=recv(m_SocServer,(char*)buffer,sizeof(float)*256,0);
...
bool clientCom::recv_data(float* data_array)
{
    int val=recv(m_SocServer,(char*)data_array,sizeof(float)*256,0);
