...
bool serverCom::send_data(string data_string)//send to all clients
{
    ...
    for(int i=0;i<(int)m_vSocClients.size();i++)
    {
        ...
        if(val!=(int)data_string.length()+1) error_flag=true;//received wrong amount
    }
...
bool serverCom::send_data(float* data_array)//send to all clients
{
    bool error_flag=false;
    for(int i=0;i<(int)m_vSocClients.size();i++)
    {
        int val=send(m_vSocClients[i],(char*)data_array,sizeof(float)*data_array[0],0);

        if(val!=int(data_array[0]*sizeof(float))) error_flag=true;//received wrong amount
    }
...
bool serverCom::send_data(float* data_array,SOCKET SocReceiver)//send to specific socket
{
    int val=send(SocReceiver,(char*)data_array,sizeof(float)*data_array[0],0);

    if(val!=int(data_array[0]*sizeof(float))) return false;//sent wrong amount
...
{
    char buffer[256];
    int val=recv(SocSender,(char*)buffer,sizeof(float)*256,0);
...
bool serverCom::recv_data(float* data_array,SOCKET SocSender)
{
    int val=recv(SocSender,(char*)data_array,sizeof(float)*256,0);
